...
{
    public class RectTransformComponent : UComponent<RectTransform>
    {
        ...
        private void TryChangeList(int index)
        {
            UI me = this.Parent;
            UI parent = me.Parent;
            var listComp = parent?.GetUIComponent<UIListComponent>();
            if (listComp != null)
            {
                int curIndex = this.GetSiblingIndex();
                var ui = listComp.GetChildAt(curIndex);
                if (ui == me)
                {
                    listComp.SetChildIndex(curIndex, index);
                }
            }
        }
